...
P_NOF_INSTANCES_TO_SELECT <- 90
P_NOF_INSTANCES_TO_GENERATE <- 90
P_NOF_MUTATIONS_TO_GENERATE <- (P_GENERATION_SIZE - P_NOF_INSTANCES_TO_KEEP - P_NOF_INSTANCES_TO_GENERATE) / (P_NOF_INSTANCES_TO_KEEP + P_NOF_INSTANCES_TO_SELECT)
...
RESULTS <- foreach(RUN = 1:P_NOF_RUNS, .combine = rbind) %dopar% {
	...
	for (I in 1:P_NOF_GENERATIONS) {
		...
		ORIGIN_INDEXES <- c(1:P_NOF_INSTANCES_TO_KEEP, sample((P_NOF_INSTANCES_TO_KEEP + 1):P_GENERATION_SIZE, P_NOF_INSTANCES_TO_SELECT))
		for (J in 1:(P_NOF_INSTANCES_TO_KEEP + P_NOF_INSTANCES_TO_SELECT))
			for (K in 1:P_NOF_MUTATIONS_TO_GENERATE) {
				...
				ROUTE <- MUTATE_ROUTE(GENERATION[ORIGIN_INDEXES[J], 1:P_NOF_POINTS])
